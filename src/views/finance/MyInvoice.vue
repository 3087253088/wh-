  ...
  <div class="finance-invoice">
      ...
      <el-table :data="invoiceList" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="success" v-if="row.verifyStatus === 'unverified'" @click="handleVerify(row)">验真</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <el-dialog title="录入发票" v-model="dialogVisible" width="550px">
      <el-form :model="formData" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="金额" prop="amount">
          <el-input-number v-model="formData.amount" :precision="2" :min="0.01" placeholder="请输入金额" style="width: 100%" />
        </el-form-item>
        <el-form-item label="开票日期" prop="invoiceDate">
          <el-date-picker v-model="formData.invoiceDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" style="width: 100%" />
        </el-form-item>
        <el-form-item label="开票方" prop="drawer">
          <el-input v-model="formData.drawer" placeholder="请输入开票方名称" />
        </el-form-item>
        <el-form-item label="开票方税号" prop="drawerTaxNo">
          <el-input v-model="formData.drawerTaxNo" placeholder="请输入开票方税号" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="dialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handleSave" :loading="saveLoading">保存并验真</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog title="发票详情" v-model="viewVisible" width="550px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="开票方税号">{{ viewData.drawerTaxNo || '-' }}</el-descriptions-item>
        <el-descriptions-item label="验证状态">
          <el-tag :type="getVerifyStatusType(viewData.verifyStatus)">{{ viewData.verifyStatusText }}</el-tag>
...
const formData = reactive({
  invoiceCode: '', invoiceType: '', amount: null, invoiceDate: '', drawer: '', drawerTaxNo: ''
})
const viewData = ref({})
...
const handleSave = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const res = await invoiceApi.save(formData)
        if (res.code === 200) {
...
const resetForm = () => {
  Object.assign(formData, { invoiceCode: '', invoiceType: '', amount: null, invoiceDate: '', drawer: '', drawerTaxNo: '' })
}
...
<style scoped>
.finance-invoice { display: flex; flex-direction: column; gap: 20px; }
