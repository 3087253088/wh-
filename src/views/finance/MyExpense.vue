      ...
      <el-table :data="expenseList" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="warning" v-if="row.status === 'draft'" @click="handleEdit(row)">编辑</el-button>
            <el-button link type="danger" v-if="row.status === 'draft'" @click="handleDelete(row)">删除</el-button>
            <el-button link type="success" v-if="row.status === 'draft'" @click="handleSubmit(row)">提交</el-button>
            <el-button link type="info" v-if="row.status === 'pending'" @click="handleRecall(row)">撤回</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="600px">
      <el-form :model="formData" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="发生日期" prop="expenseDate">
          <el-date-picker v-model="formData.expenseDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" style="width: 100%" />
        </el-form-item>
...
const handleView = async (id) => {
  try {
    const res = await expenseApi.getDetail(id)
    if (res.code === 200) {
...
const handleSubmit = async (row) => {
  try {
    await ElMessageBox.confirm('确定提交该报销单吗？提交后将进入审批流程。', '提示', { type: 'info' })
...
const handleRecall = async (row) => {
  try {
    await ElMessageBox.confirm('确定撤回该报销单吗？', '提示', { type: 'warning' })
...
<style scoped>
.finance-expense { display: flex; flex-direction: column; gap: 20px; }
